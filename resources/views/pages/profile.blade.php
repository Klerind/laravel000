  ...
  <div class="col-md-6">
    ...
    <tr>
      ...
      <?php foreach ($post as $item): ?>
         <td tabindex="<?php echo $item['content_id']; ?>">{{$item['content']}}</td>
         <?php $contents_id .= $item['content_id'].','; ?>
      <?php endforeach; ?>
      <?php if ($all_fields > $all): ?>
        <?php for ($i=0; $i < $all_fields - $all; $i++) { ?>
...
<div class="col-md-6">
  ...
  <table>
   <tr>
     @foreach($fields['field_group_product'] as $field)
      <th>{{$field->field->name}}</th>
     @endforeach
     <th>added on</th>
     <th>Edit or Delete</th>
...
<?php
    ...
    <?php foreach ($product as $item): ?>
      <td tabindex="3" contentid={{$item['content_id']}}>{{$item['content']}}</td>
    <?php endforeach; ?>
    <?php if ($all_fields > $all): ?>
      <?php for ($i=0; $i < $all_fields - $all; $i++) { ?>
           <td>no content</td>
